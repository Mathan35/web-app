...
class ExperienceJob extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $categories = Job::where('start_ex', '>=', 0)->distinct()->pluck('category')->map(function ($category) {
        ...
        $jobs = $jobs->whereStatus('approved')->orderBy('created_at', 'desc')->where('start_ex', '>=', 0)->paginate(5)->withQueryString()->onEachSide(2);

        $this->loading = false;
   
        return view('livewire.experience-job', compact('jobs', 'categories'));
    }
}
